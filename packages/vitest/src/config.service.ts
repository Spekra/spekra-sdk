/**
 * Configuration service for Vitest reporter
 */

import type { LoggerService } from '@spekra/core';
import type { SpekraVitestOptions, ResolvedConfig } from './types';
...
/**
 * Environment variable names for Vitest-specific config
 */
const ENV_VARS = {
  apiKey: ['SPEKRA_VITEST_API_KEY', 'SPEKRA_API_KEY'],
  source: ['SPEKRA_VITEST_SOURCE', 'SPEKRA_SOURCE'],
  apiUrl: ['SPEKRA_VITEST_API_URL', 'SPEKRA_API_URL'],
  enabled: ['SPEKRA_VITEST_ENABLED', 'SPEKRA_ENABLED'],
  debug: ['SPEKRA_VITEST_DEBUG', 'SPEKRA_DEBUG'],
  failOnError: ['SPEKRA_VITEST_FAIL_ON_ERROR'],
};

/**
 * Configuration service for resolving and validating Vitest reporter options
 */
export class ConfigService {
   ...
   */
  resolve(options: SpekraVitestOptions): ResolvedConfig {
    return {
  ...
  isReady(config: ResolvedConfig): { ready: boolean; reason?: string } {
    ...
    if (!config.apiKey) {
      return {
        ready: false,
        reason:
          'Missing API key. Set apiKey option or SPEKRA_VITEST_API_KEY / SPEKRA_API_KEY environment variable.',
      };
    }

    // Check for source (required for Vitest)
    if (!config.source) {
      return {
        ready: false,
        reason:
          'Missing source. Set source option or SPEKRA_VITEST_SOURCE environment variable. Source is required to identify your test suite.',
      };
    }
